    ...
    static void Main(string[] args)
    {
        if (args != null)
        {
            if (args.Length < 2)
            {
                ...
                Console.WriteLine("Ex1:    Version . 1.2.3");
                Console.WriteLine("Ex2:    Version . 1.2.3 -Common");
                Console.WriteLine("Ex2:    Version . 1.2.3 1.2.3.4 -Common -External -3rdParty");
                Console.WriteLine();
            ...
            {
                Console.WriteLine("Usage: Version path <version> [<file version>] [-ignore_foler1 ignore_foler2 ...]");
                Console.WriteLine("ERROR: " + args[1] + " should have 1, 2, or 3 parts (major.minor.build)");
                return;
            }
            ...
            foreach (DirectoryInfo subfolder in folders)
            {
                ...
                if (File.Exists(filename))
                {
                    try
                    {
                        ...
                        content = Regex.Replace(content, "VERSION = \"\\d+(\\.\\d+(\\.\\d+)?)?", "VERSION = \"" + version);
