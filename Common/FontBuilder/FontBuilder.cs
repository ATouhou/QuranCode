...
public class FontBuilder
{
    ...
    private static PrivateFontCollection s_fonts;

    public static Font Build(Stream font_stream, string font_resource_name, float font_size)
    {
        InstallFont(font_stream, font_resource_name);
        if (s_fonts != null)
        {
            if (s_fonts.Families.Length > 0)
            {
                // Handy how one of the Font constructors takes a FontFamily object, huh? :-)
                return new Font(s_fonts.Families[0], font_size);
            }
        }
        return null;
    }
