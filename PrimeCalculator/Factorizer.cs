                        ...
                        while (number > 1L)
                        {
                            ...
                            else // trial divide by all odds upto sqrt(number)
                            {
                                ...
                                for (long i = 3L; i <= max; i += 2L)
                                {
                                    ...
                                    if ((i % batch_size) == 1L) // we're using odd
                                    {
                                        m_duration = DateTime.Now - m_start;
                                        m_form.BeginInvoke(UpdateProgressBarDelegate, new object[] { ((int)((long)(i * 100L) / max)) });
                                        Thread.Sleep(((MainForm)m_form).SleepTime);
                                    }
                                    ...
                                    if ((number % i) == 0L)
                                    {
                                        ...
                                        prime_factors_str += ((prime_factors_str == string.Empty) ? "" : " ") + i + ((number > 1L) ? " *" : "");
                                        m_form.BeginInvoke(UpdateFactorsTextBoxDelegate, new object[] { prime_factors_str });
                                        Thread.Sleep(((MainForm)m_form).SleepTime);
