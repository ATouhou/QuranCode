    ...
    public static string CalcCharFrequencies(string filename)
    {
        ...
        try
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                long percentage = ((((blocks - 1L) * s_cache_size) + read_data_length) * 100L) / file_size;
    ...
    public static string CalcCharRepetitions(string filename)
    {
        ...
        try
        {
            ...
            while (!reader.EndOfStream)
            {
                ...
                // skip whole number
                if ((blocks == 1L) && s_skip_whole_number)
                {
                    cache[0] = 'N';
                }
                for (int i = 0; i < read_data_length; i++)
                {
                    if ((cache[i] != 'N') && (cache[i] != '.'))
                    {
                        ...
                        {
                            current_position = ((blocks - 1L) * s_cache_size) + i - 2L;
                        }
                        else
                        {
                            current_position = ((blocks - 1L) * s_cache_size) + i;
                        }
            ...
            while (!reader.EndOfStream)
            {
                ...
                for (int i = 0; i < read_data_length; i++)
                {
                    ...
                    if (cache[i] == pattern[pattern_index]) // found start of pattern
                    {
                        ...
                        if (found == true)
                        {
                            long skip_by = s_skip_whole_number ? 2L : 0L;
                            positions.Add(((blocks - 1L) * s_cache_size) + i + 1L - pattern_index - skip_by);
                            i += pattern.Length;
                        }
                    }
